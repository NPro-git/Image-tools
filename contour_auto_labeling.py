...
png_files = [f for f in os.listdir(dir) if '.png' in f]

stride = 5
selected_max_num = 20
ignored_countour_max_len = 20
ignored_cos_th = 0.0001

for png_file in png_files:
    png_path = os.path.join(dir, png_file)
    img_mask = cv2.imread(png_path)
    hh, ww = img_mask.shape[:2]
    img, mask = img_mask[::,:ww//2], img_mask[::,ww//2:]
    mask = np.max(mask, axis=2)
    contours = getContours(mask, 128)
    contours_img = np.zeros(mask.shape, dtype=np.uint8)
    polygons_img = contours_img.copy()
    for contour in contours:
        abs_cos_list = []
        contour_len = len(contour)
        if contour_len < ignored_countour_max_len:
            continue
        for idx in range(contour_len):
            pre_idx = (idx-stride+contour_len)%contour_len
            rear_idx = (idx+stride)%contour_len
            cur_x, cur_y = contour[idx]
            pre_x, pre_y = contour[pre_idx]
            rear_x, rear_y = contour[rear_idx]
            pre_l = np.sqrt((pre_x-cur_x)**2+(pre_y-cur_y)**2)
            rear_l = np.sqrt((rear_x - cur_x) ** 2 + (rear_y - cur_y) ** 2)
            dot_product = (pre_x-cur_x)*(rear_x-cur_x) + (pre_y-cur_y)*(rear_y-cur_y)
            abs_cos = dot_product/(pre_l*rear_l)
            abs_cos_list.append(abs_cos)
        for (coor_x, coor_y), abs_cos in zip(contour, abs_cos_list):
            contours_img[coor_y, coor_x] = np.array(255*abs_cos).astype(np.uint8)
        for coor_x, coor_y in contour:
            polygons_img[coor_y, coor_x] = 100

        # nms
        selected_idcs = []
        for times in range(selected_max_num):
            idx = np.argmax(abs_cos_list)
            if np.max(abs_cos_list) < ignored_cos_th:
                break
            selected_idcs.append(idx)
            for ii in range(-stride, stride+1):
                jj = (idx+ii+contour_len)%contour_len
                abs_cos_list[jj] = 0
        for idx in selected_idcs:
            coor_x, coor_y = contour[idx]
            polygons_img[coor_y, coor_x] = 255
    ...
    cv2.imshow('mask', np.concatenate((mask, contours_img, polygons_img), axis=1))
    cv2.waitKey(200)
